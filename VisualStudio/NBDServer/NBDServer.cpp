// NBDServer.cpp : Defines the entry point for the console application.
//
...
DWORD WINAPI blockServe(LPVOID data){
	...
	else if (_strnicmp(filename, "\\\\.\\", 4 ) == 0 && !bMemory ) //assume a volume name like \\.\C: or \\.\HarddiskVolume1 
	{
		//calc length:
		GET_LENGTH_INFORMATION pLength;
		DWORD dwplSize = sizeof(GET_LENGTH_INFORMATION);
...
int main(int argc, char *argv[])
{
	...
	debug=true;
	nbdclient="192.168.52.100";
	nbdfilename="\\\\.\\PHYSICALDRIVE0";

	/*int count;
    for( count = 1; count < argc; count++ ){
		string ch = string(argv[count]).erase(0;
		//cout << "arg: " << ch << "\n";
	}
         //cout << "  argv[" << count << "]   "
           //     << argv[count] << "\n";
    ...
    switch(ch)
    {
        ...
        case 'n':
			...
			if (_strnicmp(_strdup(optarg),"all",3)==0){
				partitionNo=-1;
			}else{
